...
class NetAB(nn.Module):
    ...
    def forward(self, x):
        ...
        p = [
            self.transition[0](output2).unsqueeze(2),
            self.transition[1](output2).unsqueeze(2)
        ]
        prob = torch.cat(p, 2)
        sen_logits = clean_logits.unsqueeze(1)
        noisy_logits = sen_logits.matmul(prob)
...
class ConvModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(3):
            mid = self.cnn[i](x)
            mid = self.relu(mid)
            mid = self.max_pool[i](mid)
            hidden.append(mid)
        return torch.cat(hidden,dim=1).squeeze()
